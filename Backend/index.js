...
app.post("/saveCreditScore", async (req, res) => {
  try {
    const { creditScore, emailAddress } = req.body;

    // Find the user by email
    const existingUser = await userModel.findOne({ email: emailAddress });
    ...
    existingUser.creditScore = creditScore;
    await existingUser.save(); // Save the changes to the database

    res.status(200).json({ message: "Credit score saved successfully" });
  } catch (error) {
